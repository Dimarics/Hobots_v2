...
StackBlock {
    id: root
    readonly property real ifContentHeight: stackHeight(ifSocket)
    ...
    ScratchText { text: "то" }
    sockets: [
        Socket { id: ifSocket; x: 40; y: headerHeight; block: root },
    ...
    shapePath: ShapePath {
        ...
        PathLine { x: 16; relativeY: elseContentHeight - 8 }
        PathArc { x: 4 + 16; relativeY: 4; radiusX: 4; radiusY: 4; direction: PathArc.Counterclockwise }
        PathLine { relativeX: 9.51472; relativeY: 0 }
        PathArc { relativeX: 4.24264; relativeY: 1.4645; radiusX: 6; radiusY: 6 }
        PathLine { relativeX: 4.48528; relativeY: 4.48528 }
    ...
    function run() {
        if (condition.value) {
            if (ifSocket.next) ifSocket.next.run()
        } else {
            if (elseSocket.next) ifSocket.next.run()
        }
    }
}
